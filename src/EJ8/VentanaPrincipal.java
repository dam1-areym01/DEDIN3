...
public class VentanaPrincipal {
	...
	MiBotonColorido[][] botonesColoridos = new MiBotonColorido[4][4];
	...
	public void inicializarComponentes() {
		...
		pDerechaArriba.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createEmptyBorder(30, 30, 30, 30),
		...
		for (int i = 0; i < botones.length; i++) {
			for (int j = 0; j < botones[i].length; j++) {
				botones[i][j] = new MiBoton(this, i, j);
		...
		pDerechaAbajo.setLayout(new GridLayout(4, 4));
		settings = new GridBagConstraints();
		settings.gridx = 1;
		settings.gridy = 1;
		settings.weightx = 1;
		...
		for (int i = 0; i < botones.length; i++) {
			for (int j = 0; j < botones[i].length; j++) {
				botonesColoridos[j][i] = new MiBotonColorido(j, i);
				pDerechaAbajo.add(botonesColoridos[j][i]);
			}
		}
	 ...
	 * @return
	 */
	public MiBotonColorido getBotonColorido(int i, int j) {
		return botonesColoridos[i][j];
	}
